control process_int_source_sink (
    ...
    //TODO: Add byte counters from onos int implementation?
...
control process_int_source (
    ...
    action int_source(bit<5> hop_metadata_len, bit<8> remaining_hop_cnt, bit<4> ins_mask0003, bit<4> ins_mask0407) {
        ...
        hdr.intl4_shim.int_type = 1;                            // int_type: Hop-by-hop type (INT-MD?) (1) , destination type (2), MX-type (3)
        hdr.intl4_shim.npt = 0;                                 // next protocol type: 0
        hdr.intl4_shim.len = INT_HEADER_LEN_WORD;               // This is 3 from 0xC (INT_TOTAL_HEADER_SIZE >> 2) (3words*4bytes = 12 bytes)
        ...
        hdr.int_header.instruction_mask_0811 = 0; // not supported: TODO: see if this can be added with v1model queing metadata
        ...
        // add the total int header len (3 words of int header) + (1 word of int shim header) to total len (3words*4B + 1*4B =16bytes))
