 ...
 */
package org.custominbandtelemetry;
...
public class CustomInbandTelemetry implements IntService {


    private final Logger log = LoggerFactory.getLogger(getClass());
    private static final int CONFIG_EVENT_DELAY = 2; // Seconds.
    private static final String APP_NAME = "org.custominbandtelemetry.app";
    ...
    protected void activate()  throws IOException {
        log.info("ACTIVATING " + APP_NAME);

        final ApplicationId appId = coreService.registerApplication(APP_NAME);

        //Classes that will be serialized to optimize IO (storage or network serialization/deserialization)
        ...
        //This is related to REST API json encoder from SimpleIntManager app
        //codecService.registerCodec(IntIntent.class, new IntIntentCodec());

        //TODO: REVIEW; this initializes the devicestoconfigure listener, which might be an internal event to trigger
        // configurations when specified on the web side of the app
        ...
        //TODO: REVIEW; this initializes the intentmap and its listener, which should the triggered when a new intent
        // is stored on the map??
        ...
        //This listener triggers then the boolean atomic value of intstarted is changed at the activation of the application
        ...
        //netcfgService.applyConfig()
        IntReportConfig reportconfig = getIntReportConfig("/report-config.json");
        log.info("+++ REPORT CONFIG: "+ reportconfig.toString());

        netcfgService.applyConfig(appId, IntReportConfig.class, reportconfig.node());
    ...
    protected void deactivate() {
        log.info("DEACTIVATING " + APP_NAME);
    ...
    @Override
    public void startInt(Set<DeviceId> transitDevices) {
    ...
    @Override
    public void stopInt(Set<DeviceId> transitDevices) {
    ...
    //Private Custominbandtelemetry functions
    ...
    private void configDeviceTask(DeviceId deviceId, long creationTime) {
        ...
        try {
            ...
            //TODO: HERE THE CONFIG FUNCTION IS CALLED
    ...
    private IntReportConfig getIntReportConfig(String fileName) throws IOException {
        ...
        config.init(coreService.getAppId(APP_NAME), INT_REPORT_CONFIG_KEY, jsonNode, mapper, c -> {
        });
        return config;
    }
    ...
    /*
     * Internal defined classes, listeners which trigger device configuration
     */
    ...
    private class InternalIntConfigListener
        ...
        public void event(AtomicValueEvent<IntDeviceConfig> event) {
            log.info("+++ IntConfigListener has been triggered: " + event.toString());
            triggerAllDeviceConfigure();
        }
    }
    ...
    private class IntAppConfigListener implements NetworkConfigListener {
        ...
        public void event(NetworkConfigEvent event) {
            log.warn("+++ Network Config Event: "+event.toString());
            eventExecutor.execute(() -> {
